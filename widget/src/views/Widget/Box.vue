...
<script lang="ts">
import {
  defineComponent, computed, ComputedRef, SetupContext,
} from 'vue';
import colors from 'tailwindcss/colors';
import { DefaultColors } from 'tailwindcss/types/generated/colors.d';
...
export default defineComponent({
  ...
  setup(_, { emit }: SetupContext): SetupReturn {
    ...
    const canGoBack = computed<boolean>(() => store.currentComponent === 'SelectFeedbackType');

    const canShowAdditionalControlAndInfo = computed<boolean>(() => store.currentComponent !== 'Success' && store.currentComponent !== 'Error');
